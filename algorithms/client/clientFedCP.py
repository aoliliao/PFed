...
class clientFedcp(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.model = Ensemble(
            ...
            feature_extractor=copy.deepcopy(self.model.base)
        )
    ...
    def set_parameters(self, feature_extractor):
        for key in feature_extractor.state_dict().keys():
            self.model.model.base.state_dict()[key].data.copy_(feature_extractor.state_dict()[key])

        for key in feature_extractor.state_dict().keys():
            self.model.feature_extractor.state_dict()[key].data.copy_(feature_extractor.state_dict()[key])
